...
def play_game():
    ...
    # loading an obstacle
    obstacle = pygame.image.load('fence1.png')
    ...
    #active_sprite_list.add(animated_horse)
    # Load stars
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == pygame.K_LEFT:
                    animated_horse.go_left()
                if event.key == pygame.K_RIGHT:
                    animated_horse.go_right()
                if event.key == pygame.K_UP:
                    animated_horse.jump()
            elif event.type == pygame.KEYUP:
                if event.key == pygame.K_LEFT and animated_horse.change_x <0:
                    animated_horse.stop()
                if event.key == pygame.K_RIGHT and animated_horse.change_x > 0:
                    animated_horse.stop()

        active_sprite_list.update()
        if animated_horse.rect.right > W:
            animated_horse.rect.right = W
        if animated_horse.rect.left < 0:
            animated_horse.rect.left = 0
        active_sprite_list.draw(DISPLAYSURF)
        # display background
        DISPLAYSURF.blit(background_pic, background)
        # Display horse
        animated_horse.draw(DISPLAYSURF, index % animated_horse.totalCellCount, 75, HH + 100, CENTER_HANDLE)
        index += 1
